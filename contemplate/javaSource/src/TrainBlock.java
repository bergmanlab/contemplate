...
/**
 * This class is used to train Block objects using the Viterbi algorithm that were previously
 * created and parameterised by MakeBlock. On completion of training, the instance will write the parameters that resulted
 * from training to STDOUT.  These can be saved and used to reinstantiate a trained
 * model for further use.
 * <br><br>
 * Use as follows:<br>
 * <pre>
 * java TrainBlock &lt;Block submodel XML filename&gt; &lt;training set filename&gt; &lt;min length for training&gt;
 * </pre>
 * <p>
...
public class TrainBlock
{
    ...
    {
        if (argv.length != 3) {
            System.out.println("Usage: java TrainBlock <Block parameters filename> <training set filename> <min length for training>");
            System.exit(1);
        }
        ...
        Block thisBlock = new Block(root);
        ...
        State start = thisBlock.insertSubModel(model, model.magicalState(), model.magicalState(), Annotation.EMPTY_ANNOTATION);
