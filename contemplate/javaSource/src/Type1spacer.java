 ...
 * This class models short, constrained spacers within enhancers.
 * This class differs from Type2spacer in that it can emit spacers that 
 * consist of only gaps in one of the sequences.
 * <p>
 *
 * @author David Huen
 */
public class Type1spacer
    implements SubModelConstructor
{
    String name = "Type1spacer";
    ...
    // type 5: mismatch, single destination
    final private CommonDistributionFactory hubFact = new CommonDistributionFactory(3);
    ...
    final static String MISMATCH_TYPE1SPACER = "mismatch_type1spacer";
    ...
    final static String TYPE_5_DIST = "type5dist";
    ...
    {
        public String getKey() { return MISMATCH_TYPE1SPACER; }
        public SubModelConstructor makeConstructor(Node node)
            throws IllegalArgumentException
        {
            return new Type1spacer(node);
        }
    };
    ...
    /**
     * Instantiate the Type1spacer from a DOM subtree.
     * @param paramNode root node of the subtree parameterising this object.
     */
    public Type1spacer(Node paramNode)
        throws IllegalArgumentException
    {
        ...
        if ((paramNode.getNodeType() == Node.ELEMENT_NODE) &&
            ...
            if (((attr = attrs.getNamedItem(TYPE)) == null) ||
                !(attr.getNodeValue().equals(MISMATCH_TYPE1SPACER))
                ) {
            ...
            for (int i=0; i<children.getLength(); i++) {
                ...
                if (thisNode.getNodeType() == Node.ELEMENT_NODE) {
                    if ((tag = thisNode.getNodeName()).equals(TYPE_1_DIST)) {
                    ...
                    else if ((tag = thisNode.getNodeName()).equals(TYPE_5_DIST)) {
                        attrs = thisNode.getAttributes();
                        ...
                        hubFact.setProbabilities(probs);
                    }
...
//                    else System.out.println("tag is " + tag);
    ...
    private void initialise()
    {
        ...
        nTotalStates = nMatchOnlyStages + 3 * nMixedStages + 1;
    ...
    /**
     * When true, the initial DotState in this model is replaced
     * by a G-only state to allow the registration of the training
     * to be established even with a potentially zero-length
     * training sequence.
     */
     ...
     * @param forTraining replaces initial dot state with a G-only emission state.
     */
        ...
        throws IllegalAlphabetException, IllegalSymbolException, ChangeVetoException
    {
        ...
        // add an initial DotState
        if (forTraining) {
            Distribution gOnlyDist = new UntrainableDistribution(
                modelAlfa
                );
            gOnlyDist.setWeight(
                modelAlfa.getSymbol(
                    Arrays.asList( new Symbol [] {DNATools.g(), DNATools.g()})
                ),
                1.0
            );
            subModelStates[i++] = thisState = new SimpleEmissionState(
                name + ":G matcher (pseudo-hub):",
                userAnnotation,
                mismatchAdvance,
                gOnlyDist
            );
            ...
            subModelStates[i++] = thisState = new SimpleDotState(
                name + ":dot",
                userAnnotation
                );
            model.addState(thisState);
        ...
        // initial hub dot state
        thisState = subModelStates[i++];
        model.createTransition(thisState, mismatch);
        model.createTransition(thisState, insert0);
        model.createTransition(thisState, insert1);
        dests = new AtomicSymbol[3];
        dests[0] = mismatch;
        dests[1] = insert0;
        dests[2] = insert1;
        thisDist = hubFact.getDependentDistribution(model.transitionsFrom(thisState), dests);
        model.setWeights(thisState, thisDist);
        ...
        throws IllegalArgumentException 
   {
        ...
        )
    {
        ...
        // type 5: dot state
        prob = new double [3];
        double wSubTotal = wMatch + wInsert0 + wInsert1;
        prob[0] = wMatch/wSubTotal;
        prob[1] = wInsert0/wSubTotal;
        prob[2] = wInsert1/wSubTotal;
        hubFact.setProbabilities(prob);
    }
        ...
        throws IOException
    {
        ...
        xw.attribute(TYPE, MISMATCH_TYPE1SPACER);
        ...
        xw.openTag(TYPE_5_DIST);
        probs = hubFact.getProbabilities();
        xw.attribute(MISMATCH, Double.toString(probs[0]));
        xw.attribute(INSERT0, Double.toString(probs[1]));
        xw.attribute(INSERT1, Double.toString(probs[2]));
        xw.closeTag(TYPE_5_DIST);
